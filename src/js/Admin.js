...
export default class Admin {
  ...
  async showOrders() {
    try {
      ...
      // order total should only have 2 decimal places
      parent.innerHTML = orders.map(order=> `<tr><td>${order.id}</td><td>${new Date(order.orderDate).toLocaleDateString('en-US')}</td><td>${order.items.length}</td><td>${parseFloat(order.orderTotal).toFixed(2)}</td></tr>`).join('');
